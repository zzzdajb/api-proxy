...
app.all('/:service*', async (req, res) => {
  try {
    ...
    // 处理根路径重定向
    if (req.method === 'GET' && (req.path === `/${service}` || req.path === `/${service}/`)) {
      return res.redirect('/');
    }
    
    // 构建目标URL
    const targetPath = req.path.replace(`/${service}`, '');
    const targetURL = `https://${config.host}${targetPath}${req.url.includes('?') ? req.url.substring(req.url.indexOf('?')) : ''}`;
